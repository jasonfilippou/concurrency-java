 ...
 */
public class WaitNotifyMainApplication {
    ...
    private static class ThreadSafeQueue {
        ...
        // Run by the producer to add a pair of matrices to the queue.
        public synchronized void add(MatricesPair matricesPair) {
            while (queue.size() == CAPACITY) {
                try {
                    wait(); // Producer blocked until the queue clears up a bit.
        ...
        // Run by the consumer to pick up a pair of matrices.
        public synchronized MatricesPair remove() {
            MatricesPair matricesPair = null;
            while (isEmpty && !isTerminate) { // Remember that the reason for the while loops is to guard against spurious interrupts.
                try {
                    wait();
                } catch (InterruptedException e) {
                }
            ...
            if (queue.size() == CAPACITY - 1) { // We should wake up the producer.
                notifyAll();
            }
            return matricesPair;
        }
